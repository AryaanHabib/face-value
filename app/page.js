    ...
    <>
      ...
      <div className="wrap">
            ...
            <div className={"hero " + (hero.per100 >= 0 ? "" : "dn")}>
              <div className="top">
                <div><div className="pname">{hero.name}</div>
                  <div className="pmeta">{hero.team} · {hero.shots} FGA</div></div>
        ...
        <div className="board">
          ...
          {filtered.map((p, idx) => {
            const pos = p.per100 >= 0, w = (Math.abs(p.per100) / max) * 50;
            return (
              <Link className="brow" key={p.id} href={`/player/${p.id}`}>
                <span className="rank">{idx + 1}</span>
                <div className="bname">{p.name}<small>{p.team}</small></div>
                <div className="track">
                  <div className="zero" />
                  <div className={"bar " + (pos ? "pos" : "neg")}
                    style={pos ? { width: w + "%" } : { width: w + "%", left: (50 - w) + "%" }} />
                  <div className="bval" style={{ color: pos ? "var(--up)" : "var(--down)",
                    left: pos ? `calc(50% + ${w}% + 6px)` : `calc(${50 - w}% - 42px)` }}>
                    {pos ? "▲ +" : "▼ "}{p.per100}</div>
                </div>
              </Link>
            );
