...
export default function Player() {
  ...
  return (
    <>
      <div className="head">
        <div className="ph-id"><Head id={p.id} name={p.name} size={40} ring={pos?"var(--up)":"var(--down)"} />
      ...
      <div className="wrap">
        ...
        {p.openness_adj != null && (
          <div className="v2box">
            <div className="v2row">
              ...
              <span className="v2v">{p.per100_base >= 0 ? "+" : ""}{p.per100_base}</span>
            </div>
            <div className="v2row">
              <span className="v2k">Openness adj.
                <small>{p.openness_adj >= 0 ? "took harder shots than avg" : "lived on easier looks"}</small>
              </span>
              <span className="v2v" style={{ color: p.openness_adj >= 0 ? "var(--up)" : "var(--down)" }}>
                {p.openness_adj >= 0 ? "+" : ""}{p.openness_adj}</span>
            </div>
            <div className="v2row v2total">
              <span className="v2k">Final / 100</span>
              <span className="v2v" style={{ color: p.per100 >= 0 ? "var(--up)" : "var(--down)" }}>
                {p.per100 >= 0 ? "+" : ""}{p.per100}</span>
            </div>
          </div>
        )}

        <div className="seg">
          {[["scatter", "Every shot"], ["zones", "Hot zones"]].map(([k, l]) => (
        ...
        {mode === "scatter" && (
          <div className="chips">
            {[["all", "All"], ["made", "Made"], ["miss", "Missed"]].map(([k, l]) => (
              <button key={k} className={"qchip " + (result === k ? "on" : "")} onClick={() => setResult(k)}>{l}</button>
            ))}
            <span style={{ width: 8 }} />
            {QUAL.map(([k, l]) => (
              <button key={k} className={"qchip " + (qual.has(k) ? "on" : "")} onClick={() => toggleQ(k)}>{l}</button>
            ))}
          </div>
        )}

        <div className="court-card">
          <Court shots={mode === "zones" ? all : shots} mode={mode} interactive={mode === "scatter"} />
          <div className="ppsrow">
            <div className="pps"><div className="k">Actual</div><div className="v">{p.actual}</div></div>
            <div className="pps"><div className="k">Expected</div><div className="v">{p.expected}</div></div>
            <div className="pps"><div className="k">Gap</div>
              <div className="v" style={{ color: pos ? "var(--up)" : "var(--down)" }}>{pos ? "+" : ""}{p.diff}</div></div>
          </div>
          <div className="legend">
            {mode === "zones"
              ? (<><span>size = volume</span><span><i style={{ background: "#1FC288" }} />beat expected</span><span><i style={{ background: "#E8A33D" }} />even</span><span><i style={{ background: "#FF5B52" }} />below expected</span></>)
              : (<><span><i style={{ background: "#1FC288" }} />good shot</span><span><i style={{ background: "#E8A33D" }} />ok shot</span><span><i style={{ background: "#FF5B52" }} />bad shot</span><span><i style={{ background: "#A878E0" }} />heave</span></>)}
          </div>
        </div>
